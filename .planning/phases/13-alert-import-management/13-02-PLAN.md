...
<task type="auto">
  ...
  **Create `src/hooks/use-alerts.ts`:**
  ...
  1. `useAlertThresholds(guildId, filters)` — `useInfiniteQuery`:
  ...
  4. `useDeleteThreshold(guildId)` — `useMutation`:
     - DELETE to `/api/guilds/${guildId}/groups/${groupId}/alert-thresholds/${thresholdId}`
     - On success: invalidate `['guild', guildId, 'alerts']`, show success toast
     - On error: show error toast

  5. `useToggleThreshold(guildId)` — `useMutation`:
     - PATCH to `/api/guilds/${guildId}/groups/${groupId}/alert-thresholds/${thresholdId}` with `{ enabled: boolean }`
     - On success: invalidate `['guild', guildId, 'alerts']` — NO optimistic update (per locked decision: waits for API confirmation)
     - On error: show error toast

  6. `useUpdateAlertSettings(guildId)` — `useMutation`:
  ...
  7. `useBulkToggleThresholds(guildId)` — `useMutation`:
  ...
  8. `useBulkDeleteThresholds(guildId)` — `useMutation`:
  ...
  **Create `src/hooks/use-import.ts`:**
  ...
  1. `useImportTemplate(guildId)` — function (not a hook) that triggers download:
  ...
  3. `useConfirmImport(guildId)` — custom hook with POST-SSE streaming:
     ...
     - Return `{ confirm, isConfirming, error }` — `confirm` is the trigger function
  ...
  **Update `src/hooks/use-exports.ts`:**
  ...
  - No functional changes needed — the existing hooks already forward `dataType` to the API; the type extension in export.ts already widens the accepted values
...
<task type="auto">
  ...
  **Create `src/components/manage/manage-nav.tsx`:**
  ...
  - Active state detection using `usePathname()` — highlight active item with `bg-accent-purple text-white` (same pattern as sidebar)
  ...
  **Update `src/components/layout/sidebar.tsx`:**
  ...
  2. In the component body, call `const { user } = useUser()`
  3. Determine admin status: `const isGuildAdmin = user?.guilds?.find(g => g.id === guildId) && (Number(user.guilds.find(g => g.id === guildId)!.permissions) & 0x8) !== 0`
  4. After the existing guild nav items (Deleted Items link), add a conditional block:
     ```
     ...
     ```
  5. Use the same `cn()` active state pattern as existing nav items: `pathname.includes('/manage/alerts')` and `pathname.includes('/manage/data')`
...
<verification>
- `npx tsc --noEmit` passes — all hooks and components type-clean
