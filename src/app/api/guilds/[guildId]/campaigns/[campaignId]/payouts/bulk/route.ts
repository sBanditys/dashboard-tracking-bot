...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns/${campaignId}/payouts/bulk`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'process payments')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('process payments'), { status: 500 })
  }
}
