...
<task type="auto">
  ...
  <action>
    ...
    **AUTH-01 backend prerequisite (verified — already implemented):**
    The backend refresh token infrastructure this task depends on is fully implemented:
    - `DashboardSession` Prisma model stores hashed refresh tokens with 30-day TTL
    - `jwtService.ts` exports `refreshTokens()` with rotate-every-use + transaction-based atomic claim
    - `POST /api/v1/auth/refresh` endpoint in `auth.ts` reads refresh token from httpOnly cookie, validates, rotates, and sets new cookies
    - Per-token rate limiting protects the refresh endpoint
    This task focuses on the **dashboard client-side** UX: calling `/api/auth/refresh` on 401, showing toast on failure, and preserving return URL.
