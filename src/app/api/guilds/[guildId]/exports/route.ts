...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/exports`, {
      method: 'POST',
      headers: {
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/exports${queryString}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
