...
must_haves:
  truths:
    ...
    - "Generic next-page fetch failure (after retries exhausted) shows inline retry button"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/(dashboard)/guilds/[guildId]/accounts/page.tsx"
      ...
      via: "consumes error/isError/isCursorInvalid for stale cursor and retry UI"
      pattern: "isCursorInvalid|isError.*fetchNextPage"
...
**Inline retry button for general fetch failures (both pages):**

9. After the stale cursor block on accounts/page.tsx, add a third UI block for generic next-page fetch failures (network errors, 500s, etc.) that have exhausted React Query's retry attempts. Also destructure `fetchNextPage` from `useAccountsInfinite` if not already present on the accounts page:
   ```tsx
...
10. Apply the identical inline retry block to posts/page.tsx after its stale cursor block, using its already-destructured `fetchNextPage`.

This covers the locked decision: "Auto-retry next-page fetches 2-3 times on failure, then show inline retry button at the bottom." The `retry: 2` in Task 1 handles the auto-retry part; this block provides the recovery UI after retries are exhausted.
  </action>
  <verify>
    <automated>cd /Users/gabrielleal/Desktop/dashboard-tracking-bot && npx tsc --noEmit 2>&1 | head -40</automated>
  ...
  <done>
    ...
    - Both pages show inline "Try again" retry button when a generic fetch error exhausts retries (non-cursor errors)
    - Stale cursor reset button calls queryClient.resetQueries to restart from first page
    - Inline retry button calls fetchNextPage() to re-attempt the failed page
    - TypeScript compiles without errors
  </done>
</task>
