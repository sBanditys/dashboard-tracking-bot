...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns/${campaignId}/payouts/history${queryString}`, {
      headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'load payout history')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('load payout history'), { status: 500 })
  }
}
