...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns${queryString}`, {
      headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'load campaigns')
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    const data = await response.json()
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'create campaign')
      return NextResponse.json(sanitized, { status: response.status })
    }
