...
must_haves:
  truths:
    ...
    - "Admin sees record count matching filters before starting export"
    - "Admin sees real-time progress bar during export generation"
    - "Admin can cancel an in-progress export via client-side SSE abort"
...
<task type="auto">
  ...
  **Create `src/components/import-export/export-tab.tsx`:**
  ...
  7. **Progress section:** When export is in progress, show:
     ...
     - **Cancel button** (locked decision) — implements **client-side abort only** (no backend cancel endpoint needed):
       - The `useExportProgress` hook uses `EventSource` for SSE streaming. Store the `EventSource` instance in a ref.
       - When user clicks Cancel, call `eventSource.close()` to terminate the SSE connection.
       - Set local state to `cancelled`, display "Export cancelled" message with a "Start new export" button.
       - The backend export will complete in the background (the user simply stops waiting for it). The completed export will still appear in export history and be downloadable.
       - Reset the progress UI state so the user can start a new export.
