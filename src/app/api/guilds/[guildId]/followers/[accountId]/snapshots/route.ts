...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(
      `${API_URL}/api/v1/guilds/${guildId}/accounts/${accountId}/follower-snapshots${queryString}`,
      {
        headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'load follower snapshots')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('load follower snapshots'), { status: 500 })
  }
}
