...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns/${campaignId}/analytics${queryString}`, {
      headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'load campaign analytics')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('load campaign analytics'), { status: 500 })
  }
}
