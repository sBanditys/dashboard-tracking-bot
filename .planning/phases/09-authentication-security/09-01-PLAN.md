...
must_haves:
  truths:
    - "AUTH-01: Refresh token persistence is already fully implemented — DashboardSession table stores hashed refresh tokens with 30-day TTL, jwtService.ts has createTokenPair/refreshTokens with rotate-every-use + transaction-based atomic claim, auth.ts has POST /api/v1/auth/refresh endpoint. No new backend work needed for AUTH-01."
  ...
  artifacts:
    - path: "~/Desktop/Tracking Data Bot/shared/prisma/schema.prisma"
      provides: "AUTH-01: DashboardSession model with refreshToken (hashed), userId, expiresAt, revokedAt — already exists"
      contains: "DashboardSession"
    - path: "~/Desktop/Tracking Data Bot/api/src/services/dashboard/jwtService.ts"
      provides: "AUTH-01: createTokenPair (generates access+refresh, stores in DB with 30-day TTL), refreshTokens (rotate-every-use with transaction-based claim, context binding, replay detection), revokeRefreshToken, revokeAllUserSessions, cleanupExpiredSessions — already exists"
      contains: "refreshTokens"
    - path: "~/Desktop/Tracking Data Bot/api/src/routes/dashboard/auth.ts"
      provides: "AUTH-01: POST /api/v1/auth/refresh endpoint (reads refresh token from cookie, per-token rate limiting, calls refreshTokens, sets new cookies) — already exists"
    ...
    - path: "~/Desktop/Tracking Data Bot/api/src/middleware/dashboardAuth.ts"
      ...
      contains: "email_verified"
  key_links:
    - from: "jwtService.ts createTokenPair"
      to: "auth.ts /refresh endpoint"
      via: "refreshTokens() called from POST /api/v1/auth/refresh with rotate-every-use rotation"
