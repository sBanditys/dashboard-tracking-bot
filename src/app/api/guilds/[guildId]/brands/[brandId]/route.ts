...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  const { guildId, brandId } = await params
  ...
  try {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'delete brand')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('delete brand'), { status: 500 })
  }
}
