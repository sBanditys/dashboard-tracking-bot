...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await backendFetch(`${API_URL}/api/v1/guilds/${guildId}/campaigns/${campaignId}/export`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'start export')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data, { status: response.status })
  } catch {
    return NextResponse.json(internalError('start export'), { status: 500 })
  }
}
