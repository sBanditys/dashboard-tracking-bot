...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const response = await backendFetch(
      `${API_URL}/api/v1/guilds/${guildId}/groups/${groupId}/follower-stats`,
      {
        headers: {
    ...
    if (!response.ok) {
      const sanitized = sanitizeError(response.status, data, 'load follower stats')
      return NextResponse.json(sanitized, { status: response.status })
    }
    return NextResponse.json(data)
  } catch {
    return NextResponse.json(internalError('load follower stats'), { status: 500 })
  }
}
